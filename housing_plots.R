...
fig2 <- plot_ly(housing_2020, x = ~month_date_yyyymm) %>% 
  add_trace(y = ~median_listing_price_yy, name = ~'Change relative to 2019',mode = 'lines') %>%
  add_trace(y = ~median_listing_price_mm, name = ~'Change month-to-month',mode = 'lines') %>%
  layout(
...
fig3 <- plot_ly(housing_2020, x = ~month_date_yyyymm) %>% 
  add_trace(y = ~active_listing_count_yy, name = ~'Change relative to 2019',mode = 'lines') %>%
  add_trace(y = ~active_listing_count_mm, name = ~'Change month-to-month',mode = 'lines') %>%
  layout(
...
fig4 <- plot_ly(housing_2020, x = ~month_date_yyyymm) %>% 
  add_trace(y = ~median_days_on_market_yy, name = ~'Change relative to 2019',mode = 'lines') %>%
  add_trace(y = ~median_days_on_market_mm, name = ~'Change month-to-month',mode = 'lines') %>%
  layout(
...
fig5 <- plot_ly(housing_2020, x = ~month_date_yyyymm) %>% 
  add_trace(y = ~new_listing_count_yy, name = ~'Change relative to 2019',mode = 'lines') %>%
  add_trace(y = ~new_listing_count_mm, name = ~'Change month-to-month',mode = 'lines') %>%
  layout(
